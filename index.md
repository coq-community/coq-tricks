...
- `deex` (see [Deex.v](https://github.com/coq-community/coq-tricks/blob/main/src/Deex.v)) is a useful tactic for extracting the witness from an `exists` hypothesis while preserving the name of the witness and hypothesis.
...
- `learn` approach - see [Learn.v](https://github.com/coq-community/coq-tricks/blob/main/src/Learn.v) for a self-contained example or [Clément's thesis](http://pit-claudel.fr/clement/MSc/#org036d20e) for more details
...
- `constructor` and `econstructor` backtrack over the constructors over an inductive, which lets you do fun things exploring the constructors of an inductive type. See [Constructors.v](https://github.com/coq-community/coq-tricks/blob/main/src/Constructors.v) for some demonstrations.
...
- `rew H in t` notation to use `eq_rect` for a (safe) "type cast". Need to
  import `EqNotations` - see [RewNotation.v](https://github.com/coq-community/coq-tricks/blob/main/src/RewNotation.v) for a working
  example.
- `intro`-patterns can be combined in a non-trivial way: `intros [=->%lemma]` -- see [IntroPatterns.v](https://github.com/coq-community/coq-tricks/blob/main/src/IntroPatterns.v).
...
- One way to implement a tactic that sleeps for n seconds is in [Sleep.v](https://github.com/coq-community/coq-tricks/blob/main/src/Sleep.v).
...
- Defining tactics (`Tactic Notation`s) that accept multiple optional parameters directly is cumbersome, but it can be done more flexibly using Ltac2. An example can be found in [TacticNotationOptionalParams.v](https://github.com/coq-community/coq-tricks/blob/main/src/TacticNotationOptionalParams.v).
...
- To make an evar an explicit goal, you can use this trick: `unshelve (instantiate (1:=_))`. The way this work is to instantiate the evar with a fresh evar (created due to the `_`) and then unshelve that evar, making it an explicit goal. See [UnshelveInstantiate.v](https://github.com/coq-community/coq-tricks/blob/main/src/UnshelveInstantiate.v) for a working example.
...
- You can use `context E [x]` to bind a context variable, and then `let e := eval context E [y] in ...` to substitute back into the context. See
  [Context.v](https://github.com/coq-community/coq-tricks/blob/main/src/Context.v) for an example.
- If you have a second-order match (using `@?z x`, which bind `z` to a function) and you want to apply the function, there's a trick involving a seemingly useless match. See [LtacGallinaApplication.v](https://github.com/coq-community/coq-tricks/blob/main/src/LtacGallinaApplication.v) for an example.
...
- `Function` vernacular provides a more advanced way to define recursive functions, which removes the restriction of having a structurally decreasing argument; you just need to specify a well-founded relation or a decreasing measure maps to a nat, then prove all necessary obligations to show this function can terminate. See [manual](https://coq.inria.fr/doc/V8.19.0/refman/using/libraries/funind.html#coq:cmd.Function) and examples in [Function.v](https://github.com/coq-community/coq-tricks/blob/main/src/Function.v) for more details.
...
- Records fields can be defined with `:>`, which make that field accessor a coercion. There are three ways to use this (since there are three types of coercion classes). See [Coercions.v](https://github.com/coq-community/coq-tricks/blob/main/src/Coercions.v) for some concrete examples.
...
- The type classes in RelationClasses are useful but can be repetitive to prove. [RelationInstances.v](https://github.com/coq-community/coq-tricks/blob/main/src/RelationInstances.v) goes through a few ways of making these more convenient, and why you would want to do so (basically you can make `reflexivity`, `transitivity`, and `symmetry` more powerful).
- The types of inductives can be definitions, as long as they expand to an "arity" (a function type ending in `Prop`, `Set`, or `Type`). See [ArityDefinition.v](https://github.com/coq-community/coq-tricks/blob/main/src/ArityDefinition.v).
- "Views" are a programming pattern that can be used to perform a case analysis only on "relevant" parts of a datatype given the context of the case analysis. See [Views.v](https://github.com/coq-community/coq-tricks/blob/main/src/Views.v).
- Record fields that are functions can be written in definition-style syntax with the parameters bound after the record name, eg `{| func x y := x + y; |}` (see [RecordFunction.v](https://github.com/coq-community/coq-tricks/blob/main/src/RecordFunction.v) for a complete example).
...
- If you do nasty dependent pattern matches or use `inversion` on a goal and it produces equalities of `existT`'s, you may benefit from small inversions, described in this [blog post](http://gallium.inria.fr/blog/a-new-Coq-tactic-for-inversion/). While the small inversion tactic is still not available anywhere I can find, some support is built in to Coq's match return type inference; see [SmallInversions.v](https://github.com/coq-community/coq-tricks/blob/main/src/SmallInversions.v) for examples of how to use that.
- You can use tactics-in-terms with notations to write function-like definitions that are written in Ltac. For example, you can use this facility to write macros that inspect and transform Gallina terms, producing theorem statements and optionally their proofs automatically. A simple example is given in [DefEquality.v](https://github.com/coq-community/coq-tricks/blob/main/src/DefEquality.v) of writing a function that produces an equality for unfolding a definition.
- Notations can be dangerous since they by default have global scope and are imported by `Import`, with no way to selectively import. A pattern I now use by default to make notations controllable is to define every notation in a module with a scope; see [NotationModule.v](https://github.com/coq-community/coq-tricks/blob/main/src/NotationModule.v).
...
- Coq has a module system, modeled after ML (eg, the one used in OCaml). You can
  see some simple examples of using it in [Modules.v](https://github.com/coq-community/coq-tricks/blob/main/src/Modules.v). In user
...
- `Set Printing Projections` is nice in theory if you use records, but it
  interacts poorly with classes. See [Projections.v](https://github.com/coq-community/coq-tricks/blob/main/src/Projections.v).

## Other Coq commands

- `Search` vernacular variants; see [Search.v](https://github.com/coq-community/coq-tricks/blob/main/src/Search.v) for examples.
...
- Sections are a powerful way to write a collection of definitions and lemmas that all take the same generic arguments. Here are some tricks for working with sections, which are illustrated in [Sections.v](https://github.com/coq-community/coq-tricks/blob/main/src/Sections.v):
...
- Hints can now be set to global, export, or local. Global (the current default) means the hint applies to any module that transitivity imports this one; export makes the hint visible only if the caller imports this module directly. The behavior will eventually change for hints at the top-level so that they become export instead of global (see <https://github.com/coq/coq/pull/13384>), so this might be worth understanding now. [HintLocality.v](https://github.com/coq-community/coq-tricks/blob/main/src/HintLocality.v) walks through what the three levels do.
...
- Ltac is provided as a plugin loaded by the standard library; if you have `-noinit` to load it you need `Declare ML Module "ltac_plugin".` (see [NoInit.v](https://github.com/coq-community/coq-tricks/blob/main/src/NoInit.v)).
